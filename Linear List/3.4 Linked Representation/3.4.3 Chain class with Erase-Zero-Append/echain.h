...
class Chain
   ...
   public:
      ...
      void Zero() {first = 0;}// set the first pointer to zero but do not delete any node
      void Erase();// delete all nodes
      Chain<T>& Append(const T& x);

   private:
      ChainNode<T> *first, *last;// pointer to first node and last node
	};
