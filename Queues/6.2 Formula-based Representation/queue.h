...
class Queue
{
	...
	private:
		int front;   // one counterclockwise from first (0->MaxSize-1)
		int rear;    // last element (0->MaxSize-1)
...
{
	// Delete first element and put in x.  Throw OutOfBounds exception if the queue is empty.
	if (IsEmpty()) throw OutOfBounds();
	x = queue[front];
