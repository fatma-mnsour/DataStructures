...
class LinearList
	...
	public:
      ...
      LinearList<T>& Delete(int k, T& x);// delete k'th element and return in x
...
bool LinearList<T>::Find(int k, T& x) const
	{
   // Set x to the k'th element of the list. Return false if no k'th; true otherwise.
...
int LinearList<T>::Search(const T& x) const
	{
   // Locate x.  Return position of x if found. Return 0 if x not in list.
...
LinearList<T>& LinearList<T>::Delete(int k, T& x)
   ...
   if (Find(k, x))
      ...
      for (int i = k; i < length; i++)
         element[i-1] = element[i];// move elements k+1, ..., down
      length--;
      return *this;
      }
   else
...
LinearList<T>& LinearList<T>::Insert(int k, const T& x)
   ...
   if (k < 0 || k > length) throw OutOfBounds();//Throw OutOfBounds exception if no k'th element.
   if (length == MaxSize) throw NoMem();// Throw NoMem exception if list is already full.

   for (int i = length-1; i >= k; i--)
      element[i+1] = element[i];// move one up
   element[k] = x;
   length++;
   return *this;
	}
